...
const ProjectCard = ({ data }: ProjectCardProps) => {
  return (
    <article className="bg-gray-100 w-full md:w-[20rem] rounded-xl overflow-hidden shadow-md hover:shadow-lg transition-shadow duration-300">
      <Image
        src={data.image}
        alt={data.name}
        width={200}
      ...
      <div className="p-5">
        ...
        <p className="text-gray-700 text-sm mb-5 line-clamp-5 h-[100px] text-justify">
          {data.description}
        </p>
        ...
        <ul
          ...
          {data.keywords.map((keyword, index) => (
            <li
              key={index}
              className="bg-black/70 px-4 py-1 rounded-full text-white text-sm">
              {keyword}
            </li>
          ))}
        </ul>
